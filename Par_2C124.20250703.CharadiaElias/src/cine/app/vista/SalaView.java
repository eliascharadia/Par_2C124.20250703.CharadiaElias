        ...
        for (int i = 0; i < sala.getButacas().length; i++) {
            for (int j = 0; j < sala.getButacas().length; j++) {
                ...
                confirmarCompra.setOnAction(e ->{
                    but.setEstado(true);
                    String nButaca = campoIngresarNumeroButaca.getText();
...
// Agregar el GridPane a tu escena
        getChildren().addAll(nombrePelicula,gridPane,ingreseButaca,campoIngresarNumeroButaca,confirmarCompra,salir);
